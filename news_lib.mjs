...
// ---- RSS feeds (news quốc tế + VN mặc định) ----
const NEWS_FEEDS = ['https://techcrunch.com/category/artificial-intelligence/feed/', 'https://venturebeat.com/category/ai/feed/', 'https://www.theverge.com/rss/index.xml', 'https://feeds.arstechnica.com/arstechnica/technology-lab', 'https://the-decoder.com/feed/', 'https://www.technologyreview.com/feed/', 'https://www.wired.com/feed/tag/ai/latest/rss'];
const VN_FEEDS = ['https://vnexpress.net/rss/so-hoa.rss', 'https://vnexpress.net/rss/khoa-hoc.rss', 'https://vnexpress.net/rss/kinh-doanh.rss', 'https://genk.vn/rss/home.rss', 'https://cafef.vn/trang-chu.rss', 'https://tuoitre.vn/rss/nhip-song-so.rss', 'https://thanhnien.vn/rss/cong-nghe.rss'];
export function feedsFor(brand) {
  const f = brand && brand.feeds && Array.isArray(brand.feeds) ? brand.feeds.filter(Boolean) : [];
  if (f.length) return f;
  return (brand && brand.region === 'vn') ? VN_FEEDS : NEWS_FEEDS;
}
...
export async function huntNews(brand, limit = 10) {
  ...
  const rssItems = [];
  for (const feed of feedsFor(brand)) tasks.push((async () => { try { for (const it of await getRss(feed, minTs)) { add(it.title, it.link, it.ts, { rss: 1, _src: 'rss' }); rssItems.push({ title: it.title, link: it.link, w: titleWords(it.title) }); } } catch (e) {} })());
